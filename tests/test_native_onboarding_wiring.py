...
def test_welcome_explains_native_text_limit_and_does_not_gate_access():
    source = Path("bot.py").read_text(encoding="utf-8")
    block = source.split("async def _process_new_member", 1)[1].split("async def on_member_join", 1)[0]
    assert "Discord cannot collect free-text answers" in block
    assert "not checked against the roster" in block
...
def test_members_json_is_discord_based_not_roster_based():
    source = Path("bot.py").read_text(encoding="utf-8")
    block = source.split("async def _build_members_json", 1)[1].split("async def _open_game_name", 1)[0]
